  ...
  return (
    <Card>
      ...
      <form onSubmit={handleSubmit} className="form-row" style={{ gridTemplateColumns: '1fr auto' }}>
        <Input label="Nama Peserta" id="personName" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Andi"/>
        <Button type="submit">Tambah</Button>
      </form>
      <div style={{ marginTop: '1.5rem', minHeight: '50px' }}>
        <AnimatePresence>
          {people.map((person) => (
            <motion.div key={person.id} layout initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }} transition={{ duration: 0.3 }} className="list-item">
